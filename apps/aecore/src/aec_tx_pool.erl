...
-spec get_max_nonce(pubkey()) -> non_neg_integer() | undefined.
...
-spec int_get_max_nonce(pool_db(), pubkey()) -> non_neg_integer().
int_get_max_nonce(Mempool, Sender) ->
    MaxNonce = lists:max(lists:flatten(ets:match(Mempool, ?KEY_NONCE_PATTERN(Sender)))),
    {ok, MaxNonce}.
...
exclude_coinbase({undefined, undefined, _}) ->
